...
++  nice-da
  ...
  |=  d/@da
  `@da`(sub d (div (dis d ~s0..fffe) 2))
::
++  md-to-post
  |=  a/@t
  ^-  topic:collections
  =/  med  (parse:frontmatter (to-wain:format a))
  :+  (fall (~(get by -.med) %title) '')
    ::*@p
    =/  aut  (~(get by -.med) %author)
    ?~  aut
      ~zod
    ^-  @p
    (slav %p (need aut))
  (to-wain:format mud.med)
++  md-to-comm
  |=  a/@t
  ^-  comment:collections
  =/  txs/(pole @t) 
  ...
  ?>  ?=([who=@t wat=*] txs)
  =/  aut
  `@p`(rash who.txs (ifix [(jest '## `~') (jest '`')] fed:ag))
  ~&  :-  aut
  wat.txs
  :-  aut
  wat.txs
